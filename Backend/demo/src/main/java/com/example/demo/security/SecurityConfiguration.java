...
//		.antMatchers(HttpMethod.POST, "/user") // doesn't work because of deprecated
		.requestMatchers(HttpMethod.POST, "/user", "/petition", "/donations", "/stat")								// allowed posts
		.permitAll()
	    .and()
		.authorizeHttpRequests()
	    .requestMatchers(HttpMethod.GET, "/tip", "/like/findByTipId","/getPetitionSigns", "/questions", "/stat")	// allowed gets
		.permitAll()
	    .and()
//	    .authorizeRequests()  // deprecated
	    .authorizeHttpRequests()
